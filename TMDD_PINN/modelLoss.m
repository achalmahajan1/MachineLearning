function [loss,gradients] = modelLoss(parameters,t,t0,T0,C0,D0)
...
Pred = model(parameters,t);% Calculate XT*weights + bias at each layer, doing the forward propagation
T = Pred(1,:);
C = Pred(2,:);
D = Pred(3,:);

% Calculate derivatives with respect to time t.
...
% Calculate lossF. Enforce TMDD set of equation.
